export module AoC_2023.Day24;

import std;
import utils;

export std::int64_t Solve_1(const std::filesystem::path& input);
export std::int64_t Solve_2(const std::filesystem::path& input);
    ...
    for (size_t i = 1; i < stones.size(); ++i)
    {
        for (size_t j = 0; j < i; ++j)
        {
            ...
            if (auto cp = RayIntersection(p1, v1, p2, v2))
            {
                ...
                //std::cout << std::format("Intersection at {},{}\n", cp->x, cp->y);
            }
        }
    }
    return count;
