...
/*
This is a copy of standard z3++.h, slightly modified to work with modules
*/
...
export namespace z3
{
    ...
    void set_param(char const* param, char const* value) { Z3_global_param_set(param, value); }
    void set_param(char const* param, bool value) { Z3_global_param_set(param, value ? "true" : "false"); }
    void set_param(char const* param, int value) { auto str = std::to_string(value); Z3_global_param_set(param, str.c_str()); }
    void reset_params() { Z3_global_param_reset_all(); }
    void assert(bool) {}
        ...
        */
        int64_t get_numeral_int64() const {
            ...
            if (!is_numeral_i64(result)) {
                assert(ctx().enable_exceptions());
                if (!ctx().enable_exceptions()) return 0;
                throw exception("numeral does not fit in machine int64_t");
            }
            return result;
        }
        ...
        */
        uint64_t get_numeral_uint64() const {
            ...
            if (!is_numeral_u64(result)) {
                assert(ctx().enable_exceptions());
                if (!ctx().enable_exceptions()) return 0;
                throw exception("numeral does not fit in machine uint64_t");
            }
            return result;
        }
...
#undef _Z3_MK_UN_
    ...
    */
    ...
    public:
        parameter(func_decl const& d, unsigned idx) : m_decl(d), m_index(idx) {
            if (ctx().enable_exceptions() && idx >= d.num_parameters())
                throw exception("parameter index is out of bounds");
    ...
    inline tactic repeat(tactic const& t, unsigned max = std::numeric_limits<unsigned>::max()) {
...
#define MK_EXPR2(_fn, _arg1, _arg2)             \
    ...
    inline expr expr::substitute(func_decl_vector const& funs, expr_vector const& dst) {
        ...
        if (dst.size() != funs.size()) {
            throw exception("length of argument lists don't align");
